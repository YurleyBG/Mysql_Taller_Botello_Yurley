...
select *, 2024-year(fecha_nacimiento) as edad from alumno order by 2024-year(fecha_nacimiento) asc limit 1 ;
...
select profesor.nombre from profesor inner join departamento  on profesor.id_departamento = departamento.id  left join asignatura a  on profesor.id = a.id_profesor where a.id_profesor is null  ;
...
select  d.nombre,profesor.apellido1,profesor.apellido2, profesor.nombre  from profesor  left join departamento d on profesor.id_departamento=d.id order by 1,2,3,4 asc;
